...
export default function Header() {
  ...
  const { jobsInputValues, jobPage } = useContext(JobAPIContext);
  ...
  return (
    <header className="headerBackground w-full min-w-[375px] h-[136px] bg-header-mobile bg-center bg-cover mx-auto flex flex-col justify-center items-center relative">
      ...
      <div
        className={`${
          theme ? background.dark.veryDarkBlue : background.light.white
        } ${
          jobPage == 0 ? "block" : "hidden"
        } absolute bottom-0 rounded-lg translate-y-[50%] w-[327px] h-[80px] p-4 shadow-md flex flex-row justify-between items-center md:w-[689px]`}
      >
        <InputFilterByTitle />
